...
public class Rekening implements IRekeningTbvBank {
    ...
    Rekening(int number, IKlant klant, Money saldo) {
        ...
        this.eigenaar = klant;
        this.saldo = saldo;
        this.basicPublisher = new BasicPublisher(new String[]{"Rekening"});
    }
    ...
    public boolean muteer(Money bedrag) {
        ...
        if (isTransferPossible(bedrag)) {
            Money huidigeSaldo = this.saldo;
            saldo = Money.sum(saldo, bedrag);
            inform(bedrag, huidigeSaldo);
            return true;
        }
        return false;
    }
    ...
    public void inform(Money newBedrag, Money oudSaldo){
        basicPublisher.inform(this, "Rekening", oudSaldo, newBedrag);
    }
}
